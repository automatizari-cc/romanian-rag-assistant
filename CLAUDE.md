...
| Doc ingestion path | `POST /ingest` on `127.0.0.1:8000` — admin-only via SSH/curl. Open-WebUI's UI upload (paperclip) is **not** wired to our RAG; it lands in OWU's own store. | Bypass-OWU-RAG architecture is incomplete: chat completion is ours, upload still flows to OWU. Either build a public upload UI or document the curl-from-box flow. Tracked as open work. |
...
2. **CPX62 Part 2 — DONE (2026-04-30).** All five Part 2.x steps complete: DNS + Full(Strict) + AOP, LE cert (Apr 29 → Jul 28), AOP CA installed, CF v4 sync, Hetzner FW. 2.4c (Rate Limit on `/auth/*` 5/10s/IP/Block), 2.4d (WAF Managed Challenge on **`/login` GET** — moved from `/auth/*` after the XHR-eats-challenge bug). 2.5 smoke test green: Romanian login → Open-WebUI chat → Romanian response → curl-ingested doc → correct grounded answer. **`ENABLE_SIGNUP=false`** flipped on the box. Five auth/routing bugs fixed and committed in this session — see commit log on `silver-shim`. Cosmetic leftovers (Open-WebUI's flash-of-unauth-content on hydration, broken model avatar 404) noted but not blocking.
3. **Unblock RoLlama3.1 GGUF — now the top usability blocker.** qwen2.5:7b is in production as a stopgap and gives noticeably bad answers on Romanian RAG queries: rambling, occasional Chinese token drift, ignoring the injected context. CLAUDE.md "Things to watch" had this as deferred; smoke testing this session promoted it to first-priority because the model quality is what end users will judge the system by, more than any infra detail. Try in this order: (a) bartowski / mradermacher / unsloth GGUF mirrors of `OpenLLM-Ro/RoLlama3.1-8b-Instruct`, (b) custom `Modelfile` flow that downloads the FP16 weights from HF directly and converts to GGUF on the box, (c) escalate to OpenLLM-Ro maintainers / file an Ollama issue. As fallback, evaluate Mistral-Nemo-12B-Instruct (Apache 2.0, decent Romanian) — but it's commercial-friendly so no urgency to switch unless RoLlama stays blocked indefinitely.
4. **Fold `amd-fix.yml` + the silver-shim uncommitted edits into a tracked commit.** Real timing: before the next fresh-clone deploy on any AMD host (otherwise the new box can't reproduce the working stack and hits the bge-reranker MKL segfault). Decide between (a) merging into base `docker-compose.yml` and (b) creating `docker-compose.prod.yml` for the AMD overrides. The other dirty edits (issue-cert.sh perms fix, compose tmpfs, http2 deprecation in template) should land in the same commit.
5. **Doc-upload integration with Open-WebUI.** Architectural gap surfaced this session: Open-WebUI's chat UI upload (paperclip) puts files in OWU's own store and uses OWU's built-in RAG, not ours — so questions against a UI-uploaded doc bypass our `/v1/chat/completions` retrieval entirely. The verified-working ingest path is `curl -F file=@... http://127.0.0.1:8000/ingest` from the box (admin-only). Three options: (a) build a small public upload page that POSTs to `/ingest` through nginx (with auth + size cap + rate limit), (b) wrap the curl path in `scripts/ingest.sh` and document it as the admin-only KB-curation flow, (c) patch Open-WebUI's upload handler to forward to `/ingest`. (a) is right if the use case is multi-user uploads; (b) is right if it's a curated single-admin KB; (c) is fragile and pinned to OWU 0.5.4. Decide based on who's actually expected to add docs.
...
- **Open-WebUI's UI upload (paperclip / `+`) bypasses our RAG.** Files go to OWU's own internal store, not our Qdrant. Verified working ingest path: `curl -F file=@... http://127.0.0.1:8000/ingest` from the box (admin-only, `/ingest` is not exposed publicly via nginx). End users uploading docs through the chat UI will get answers from OWU's built-in RAG, which can be wrong (saw "Cotnari is in Ilfov" instead of Iași in 2026-04-30 testing). Open work item to either build a public upload UI that posts to `/ingest` or document the curl-from-box flow.
- **qwen2.5:7b is a stopgap.** Smoke testing 2026-04-30 showed clear quality issues against Romanian RAG queries: rambling, incorrect answers despite correct context, occasional Chinese token drift mid-response (qwen pre-training distribution leakage). RoLlama3.1 is the locked-in target model; unblocking the GGUF pull is now the top usability priority.
