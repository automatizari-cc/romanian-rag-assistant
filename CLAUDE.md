...
- **Romanian-first.** UI, prompt, model output are all Romanian. Source documents can be in any language but the assistant's response is always Romanian.
...
- **CPU-only inference.** ~30–40 seconds to first answer token on the wine corpus (CPX62, 16 vCPU EPYC, no GPU). Off-topic questions return the abstain message in ~4 seconds (no LLM call). GPU pivot would collapse this to seconds.
