...
| `96ffe3c` | nginx: desktop pill back to bottom-right; suppress OWU broken-image flash | User reported top-right placement covered OWU's chat-settings/sliders icon. Reverted desktop to `bottom: 1.5rem; right: 1.5rem`. Mobile keeps `top: 3.5rem`. Also added `img { font-size: 0 }` to the inline `<style>` to suppress the browser's broken-image placeholder + alt text — visible as a brief purple/pink fragment in the empty-chat center. **User reports the broken-image flash is STILL visible after this commit** — see open-work item; root cause not identified. (Resolved by `f4fcfee`.) |
| `f4fcfee` | nginx: ship favicon.png in /static/ to kill OWU model-avatar 404 flash | Root-cause fix for the broken-image flash that `96ffe3c`'s `img{font-size:0}` workaround couldn't suppress. OWU 0.5.4's runtime JS references `/static/favicon.png` as the model-avatar fallback (used whenever `profile_image_url` is absent, which is always on self-hosted models). Our nginx has `location ^~ /static/` as a high-priority prefix match serving only our 5 files (landing.js, login.js, owu-overrides.css, styles.css, upload.js) and 404's anything else. Every empty-chat paint hit a 404 on `/static/favicon.png`; the browser rendered its UA broken-image vector glyph (a small purple torn-document icon) for the frame between request firing and 404 arriving. The `font-size: 0` trick from `96ffe3c` didn't help because the glyph is a UA-stylesheet vector icon, not text/alt. Fix: drop OWU's `favicon.png` byte-for-byte (10,911 B, 500x500 RGBA) into `nginx/html/static/`. **CF cached the 404 with max-age=14400 per the free-plan edge rewrite trap** — after deploy, purge the URL via CF dashboard or wait ~4h for cached 404 to expire before the fix is visible to browsers that already saw the bad response. |
...
> - Terminated pod `zpx0u8x92jowmo` per their instruction. RTX 4000 Ada then went **Unavailable in EU-RO-1 under CUDA 12.4** — only A4500 + L4 in stock.
> - **Savings plan is SKU-locked** (Roman's clarification 2026-05-13: "not a dedicated hardware reservation … if you deploy a different GPU type, that would not be covered"). ~$345 of plan time unredeemable on the only SKUs available.
> - **Refund to Visa (last 4 8992) escalated.** Runpod agreed to refund unused balance to the original card rather than platform credits. Auto-pay disabled, screenshot sent 2026-05-13. Waiting on the relevant team to process. User intent stated explicitly to Runpod: winding down their service.
> - **Network volume `xeric_yellow_baboon`** (50 GB, EU-RO-1) is intact and detached. Still accruing storage cost until deleted — pending the refund clearing, then delete.
>
> **Implications for this project:**
> - **First-token latency stays at the ~38–40s CPU floor on CPX62** unless a new GPU strategy is chosen.
> - **ONNX rerank export** (open-work #1, sub-lever c) goes from "throwaway if GPU lands" to a **durable CPU win**. Phase A.1 is still the cheapest first move — 30-min HF Hub search for existing `bge-reranker-v2-m3-onnx` community exports.
> - **`docvault-gpu` pod (`100.100.50.70`) no longer exists** on the tailnet. Tailscale authkey expiry (Jul 15) is now moot.
> - **Tailscale on CPX62 stays installed** — infrastructure investment retained; dormant until a GPU plan returns.
>
> **Runpod-policy lessons worth retaining:**
> - **Savings plan does NOT reserve hardware.** It's an upfront commitment for discounted runtime on a specific GPU SKU. When the pod is terminated, capacity returns to the general pool — there's no guarantee the same SKU will be deployable in the same region afterward. Cross-SKU transfer is not supported.
> - **CUDA host-version mismatches are a real failure mode** and Runpod's recovery path is termination + redeploy under a stricter filter — which can strand a savings plan if the targeted SKU is then out of stock.
> - **Refund vs. platform-credits is negotiable.** The default Runpod offer is credits; pushing for original-card refund (with stated wind-down intent) worked on this ticket, but required escalation.
>
> **Items previously open, now closed by Runpod exit:**
...
1. **First-token latency reduction (cheap levers exhausted).** 2026-05-03 daytime cut wall-clock 75–85s → 38–40s on CPX62 (~50% net) via three levers: TOP_K 20→10 (`de434f8`), TOP_N 5→3 (`101d39f`), prompt-faithfulness tightening (`39e46bd`). Per-stage breakdown on Q4_K_M baseline (mean 10-Q A/B): **prefill 29.7s, decode 6.4s, rerank 4.1s** — prefill dominates ~5× over decode. **Cheap levers TESTED 2026-05-03 evening, both rejected** (see same-day follow-on callout):
   ...
   **Remaining levers, ranked by expected impact ÷ effort:**
   - **(c) ONNX export of bge-reranker-v2-m3** — biggest structural CPU win (~4s rerank floor → ~0.5–1s). ~2.5 days, broken into phases A–D in the same-day follow-on callout. **Now a durable CPU win** (was "throwaway if GPU lands" — after the Runpod GPU pivot died 2026-05-13, CPU is the foreseeable reality). Phase A.1 (30-min HF Hub search for existing exports) is cheapest first move; if a community export exists, it drops in cleanly.
   ...
   - **GPU pivot — currently off the table on Runpod.** Future evaluation (Hetzner GPU Cloud, Vast.ai, Lambda, or a different Runpod attempt with CUDA 12.8 filter) is deferred pending a fresh hardware decision. If/when pursued, concrete compose changes are documented in the 2026-05-03 same-day follow-on callout.
...
- **OWU model-avatar `<img>` flashes a broken-image placeholder during load — RESOLVED in `f4fcfee` (2026-05-11).** Root cause: OWU 0.5.4's runtime JS references `/static/favicon.png` as the model-avatar fallback (used whenever `profile_image_url` is absent, which is always on self-hosted models). Our nginx `location ^~ /static/` only served our 5 files and 404'd the favicon. The UA broken-image vector glyph painted for the frame between request and 404. `96ffe3c`'s `img { font-size: 0 }` couldn't suppress it because the glyph is a UA-stylesheet vector, not text/alt. Fix: ship OWU's favicon byte-for-byte (10,911 B) into `nginx/html/static/`. Watch out: **CF caches 404s with max-age=14400 on free plan**, so after a similar deploy purge the URL or wait ~4h for the cached 404 to expire.
