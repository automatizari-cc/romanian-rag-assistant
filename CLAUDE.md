...
> **🔔 Last session (2026-05-03) — surface these first thing next session:**
>
...
1. **Continue first-token latency reduction (was top priority; substantially DONE).** 2026-05-03 cut wall-clock 75–85s → 38–40s on CPX62 (~50% net) via three levers: TOP_K 20→10 (`de434f8`, rerank 9–13s → ~4s), TOP_N 5→3 (`101d39f`, prefill cut ~2.7s/chunk × 2 chunks), prompt-faithfulness tightening (`39e46bd`). Per-stage timings live in logs (`chat rid=<8hex>...`) and SSE comment trailer. **Remaining levers, ranked by expected impact ÷ effort:**
   - **Ollama `num_thread: 16`** in `ollama_chat_stream` options dict (`ingestion/app/llm.py`) — defaults to N/2 of vCPUs; full count may shave more from prefill on the dedicated 16-vCPU CPX62. Single-line change. Measure with the existing instrumentation.
   - **Q3_K_M sibling Modelfile** — half-day. `ollama create rollama3.1:Q3_K_M -f Modelfile.rollama3.1.q3` (need to write the q3 modelfile; copy `Modelfile.rollama3.1` and change FROM tag), then flip `OLLAMA_MODEL` in `.env`. Faster prefill; quality cost via 10-Q A/B (test harness still on box at `/tmp/lat_run.sh` from 2026-05-03 — re-runnable with arg `topn3-q3` etc.).
   - **ONNX export of bge-reranker-v2-m3** — biggest remaining structural win. Today's rerank floor (~4s on TOP_K=10) is the AMD-shim path; real TEI with ONNX would be sub-second. Project-sized: export the cross-encoder via `optimum.exporters.onnx`, run real TEI in place of the shim on AMD. Probably means revisiting `tei-shim/` entirely.
   - **Smaller `INGEST_CHUNK_TOKENS`** (256 vs 512) — orthogonal lever; lets you keep TOP_N higher with same prefill cost. Requires re-ingest of corpus; may interact with retrieval quality.
...
- **First-token latency on CPX62 — current floor ~38–40s, bottleneck is Ollama CPU prefill.** As of 2026-05-03, total wall-clock per query is ~38–40s (was ~75–85s before today's three-lever cut). Per-stage breakdown on a clean single-request: embed ~200ms, Qdrant search ~50ms, rerank ~4s (TOP_K=10), Ollama prefill+stream ~33s. Ollama prefill alone is ~30s on 16 EPYC vCPUs (no GPU). **When latency complaints come in, the UI is *not* the lever** — the box-side levers are: more Ollama threads (`num_thread:16`), smaller quant (Q3_K_M vs Q4_K_M), ONNX export of the reranker (escapes the AMD-shim 4s floor), or eventually GPU. Per-stage timings are in ingestion logs (`chat rid=<8hex> stage=...`) and in the SSE `: rag-timings={...}` comment line emitted before `data: [DONE]`. Open work item #1.
